...
function checkCode(){
	...
	$.ajax({
		...
		dataType:"json",
   		data:data,
		url : path+"/index/checkRandCodeAnsyn",
		success : function(data) {
			if(data.data.msg=='TRUE'&&data.data.result==1){
				...
				obj.find("#touclick-image").attr("src",path+"/index/getPassCodeNew?module=passenger&"+Math.random());
			}
		}
	});
}
function queryOrderWaitTime(datas){
	$.ajax({
		...
		dataType:"json",
   		data:datas,
		url : path+"/index/queryOrderWaitTime",
		success : function(data) {
			dialog.tip.remove();
