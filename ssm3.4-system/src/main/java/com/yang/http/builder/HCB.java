...
import com.yang.http.common.SSLs;
import com.yang.http.common.SSLs.SSLProtocolVersion;
...
public class HCB extends HttpClientBuilder {
	...
	public HCB pool(int maxTotal, int defaultMaxPerRoute)
		...
		connManager.setMaxTotal(maxTotal);// Increase max total connection to
											// $maxTotal
		connManager.setDefaultMaxPerRoute(defaultMaxPerRoute);// Increase
																// default max
																// connection
																// per route to
																// $defaultMaxPerRoute
		// connManager.setMaxPerRoute(route, max);// Increase max connections
		// for $route(eg：localhost:80) to 50
		isSetPool = true;
		return (HCB) this.setConnectionManager(connManager);
	}
